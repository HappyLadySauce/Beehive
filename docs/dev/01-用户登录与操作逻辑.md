...
sequenceDiagram
    ...
    Gateway->>AuthSvc: gRPC: Login(username, password)
    AuthSvc->>UserSvc: gRPC: GetUserByUsername(username)
    UserSvc->>DB: 查询用户信息
    DB-->>UserSvc: 返回用户数据
...
func (h *Handler) HandleConnection(w http.ResponseWriter, r *http.Request) {
    ...
    // 注册连接
    connection := h.connMgr.RegisterConnection(resp.UserId, conn)
    
    // 通知用户上线
    h.presenceService.UserOnline(resp.UserId)
    
    // 处理连接
    go h.handleClient(connection, resp.UserId)
}
