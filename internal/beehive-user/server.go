...
// NewUserServer 创建 User 服务器实例
func NewUserServer(cfg *config.Config, dbStore *store.Store) *UserServer {
	return &UserServer{
		cfg:     cfg,
		dbStore: dbStore,
	}
...
// PrepareRun 准备运行服务器
func (s *UserServer) PrepareRun() error {
	// 1. 创建 User Service 实例
	klog.Info("Creating User Service instance...")
	s.userService = service.NewService(s.dbStore)
	...
	if len(s.cfg.Etcd.Endpoints) > 0 && s.cfg.Etcd.Endpoints[0] != "" {
		...
		if err != nil {
			...
			// 生成实例 ID
			s.instanceID = fmt.Sprintf("%s-%s", "user", uuid.New().String()[:8])
...
func (s *UserServer) Run(ctx context.Context) error {
	...
	// 启动 gRPC 服务器
	go func() {
		klog.Infof("Starting User Service gRPC server on %s", s.grpcListener.Addr())
	...
	if s.httpServer != nil {
		go func() {
			klog.Infof("Starting User HTTP server on %s", s.httpServer.Addr)
...
func (s *UserServer) Shutdown() {
	...
	if s.serviceRegistry != nil && s.instanceID != "" {
		if err := s.serviceRegistry.Deregister("beehive-user", s.instanceID); err != nil {
