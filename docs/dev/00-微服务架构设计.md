...
graph TB
    ...
    subgraph Infrastructure["基础设施层"]
        ...
        ES[(Elasticsearch<br/>:9200<br/>消息全文搜索)]
    end
...
- 高性能全文检索
- 支持中文分词
- 支持复杂查询（多字段、时间范围、消息类型）

**关键 RPC**：
- `SearchMessages(user_id, keyword, limit, offset)` - 搜索用户相关消息
- `SearchUserMessages(user_id, target_user_id, keyword)` - 搜索两个用户之间的消息
- `SearchGroupMessages(group_id, keyword)` - 搜索群组消息
...
elasticsearch:
  ...
  sync_enabled: true  # 启用消息同步到 Elasticsearch
